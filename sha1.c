 ...
 * Corrected a problem which generated improper hash values on 16 bit
 * machines. Routine sha1_update changed from unsigned int to unsigned long.
 *
 * -----------------
 * Modified 8/98 by Steve Reid <sreid@sea-to-sky.net>
 * Still 100% public domain
 * Fixed overwriting of finalcount in sha1_final() (discovered by Chris Hall)
 *
 * -----------------
 ...
 *
 * -----------------
 * HMAC-SHA1 added. Also public domain.
 *
...
static int sha1_is_big_endian(void) {
    static const union { uint32_t i; char c[4]; } u = { 0x01020304 };
    return u.c[0] == 1;
}

#define blk0(i) (sha1_is_big_endian() ? block->l[i] : \
                            ...
                            const unsigned char buffer[64])
{
    ...
    /* Wipe variables */
    a = b = c = d = e = 0;
    memset(block, 0, sizeof(block_buf));
}
 ...
 *
 * Computes HMAC-SHA1(key, data) and writes 20 bytes to out.
 */
               ...
               unsigned char out[SHA1_DIGEST_SIZE])
{
    ...
    /* Outer: SHA1(K XOR opad, inner_hash) */
    sha1_init(&ctx);
